...
def generate_models(config: dict):
	...
	for item in database.keys():
		...
		if ore == "leather": 
			model_item["textures"]["layer2"] = "item/leather_chestplate_overlay"
			model_wearable["layers"]["humanoid"][0]["dyeable"] = {}
			model_wearable["layers"]["humanoid"][0]["dyeable"]["color_when_undyed"] = 10511680
		write_to_file(f"{build_resource_pack}/assets/{namespace}/models/item/{item}.json", super_json_dump(model_item), True)
		write_to_file(f"{build_resource_pack}/assets/{namespace}/models/equipment/{item}.json", super_json_dump(model_wearable), True)
